...
func createHeightMap(input []string) heightMap {
	...
	for i, line := range input {
		for j, c := range line {
			if c == 'S' {
				hm.starts = append(hm.starts, [2]int{i, j})
...
func iniFewestSteps(h, w int, start [2]int) {
...
func solvePart1(input []string) {
	...
	for _, start := range hm.starts {
		iniFewestSteps(len(hm.height), len(hm.height[0]), start)
		...
		if fewestSteps[hm.goal[0]][hm.goal[1]] < fewestStep {
			fewestStep = fewestSteps[hm.goal[0]][hm.goal[1]]
		}
	}

	fmt.Println(fewestStep)
