...
func solvePart2(moves []byte, dists []int) {
	visited := [LEN][LEN]bool{}

	hX, hY := LEN/2, LEN/2
	knotsX := [9]int{hX, hX, hX, hX, hX, hX, hX, hX, hX}
	knotsY := [9]int{hY, hY, hY, hY, hY, hY, hY, hY, hY}
	visited[hX][hY] = true
	...
	for i, move := range moves {
		...
		for j := 0; j < dist; j++ {
			...
			nextX, nextY := hX, hY
			for i := 0; i < 9; i++ {
				knotsX[i], knotsY[i] = getNextTailMove(nextX, nextY, knotsX[i], knotsY[i])
				nextX, nextY = knotsX[i], knotsY[i]
			}

			// fmt.Printf("Head: (%d, %d), Tail: (%d, %d)\n", hX, hY, tX, tY)

			visited[knotsX[8]][knotsY[8]] = true
		}
	}
